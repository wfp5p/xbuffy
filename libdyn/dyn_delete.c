...
/*	       bzero(obj->array + index*obj->el_size, obj->el_size); */
	       memset(obj->array + index*obj->el_size, 0, obj->el_size);
	  }
	  else {
	       if (obj->debug)
...
/*	  bcopy(obj->array + (index+1)*obj->el_size,
	  ...
	  memmove(obj->array + index*obj->el_size,
		  obj->array + (index+1)*obj->el_size,
		  obj->el_size*(obj->num_el - index));
...
/*	       bzero(obj->array + obj->el_size*(obj->num_el - 1), obj->el_size);*/
	       memset(obj->array + obj->el_size*(obj->num_el - 1), 0,obj->el_size);
	  }
     }
